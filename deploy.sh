...
docker build -t gcr.io/$GCP_ID/$APP_NAME:$APP_VERSION .
docker push gcr.io/$GCP_ID/$APP_NAME:$APP_VERSION
...
gcloud run deploy $APP_NAME \
  --image gcr.io/$GCP_ID/$APP_NAME:$APP_VERSION \
  --platform managed \
  --memory=256Mi --cpu=1000m \
  --max-instances=1 \
